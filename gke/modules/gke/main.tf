...
locals {
    ...
    services = toset(["compute", "container", "cloudsourcemanager"])
...
resource "google_container_cluser" "gke" {
    name = var.cluster_name
    location = var.region
    node_locations = locals.zones
...
resource "google_container_node_pool" "nodepools" {
    ...
    version = locals.master_version
    depends_on = [google_container_cluser.gke]
}
